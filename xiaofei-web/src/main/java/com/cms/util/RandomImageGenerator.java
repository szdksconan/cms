...
public class RandomImageGenerator {
	 ...
	 */
	 ...
	 private static Font getFont() {
	  ...
	  font[0] = new Font("宋体", Font.PLAIN, 24);
	  font[1] = new Font("宋体", Font.PLAIN, 24);
	  font[2] = new Font("宋体",Font.PLAIN, 24);
	  font[3] = new Font("宋体", Font.PLAIN, 24);
	  font[4] = new Font("宋体", Font.PLAIN, 24);
	 ...
	 public static void render(String randomStr,OutputStream out,int width,int height) throws IOException{
	  ...
	  for(int i=0;i<str1.length;i++){
	   ...
	   if(x==random.nextInt(7)){
	    w=25-random.nextInt(7);
	   }else{
	    w=20+random.nextInt(7);
	   }
	   w=23;
	   //随机生成颜色
	   g.setColor(getRandColor());
	   g.drawString(str1[i], 13*i+7, w);
	  }
