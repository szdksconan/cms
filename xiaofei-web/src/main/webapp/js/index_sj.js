		...
		success:function(data){
			...
			$.each(data,function(i,n){
				...
				if(typesum<4&&layer<2&&id==n.superid){
					typesum++;
					classlayerhtml+="<li  class=\"active\"><a  href=\"javascript:void(0);\">"+n.name+"</a></li>";
				}
		...
		success:function(data){
			...
			if(data.PageProduce!=null){
				$.each(data.PageProduce.records,function(i,n){
					goodshtml+="<span>";
					goodshtml+="<a href=\""+context+"/index_cp1.jsp?sId="+n.goodsId+"&pId="+n.productId+"&shopId="+n.enterpriceId+"\" target=\"_self\">";
...
window.get=function (){
	if($("#seekKey").val()!=""){
		 key=$("#seekKey").val();
		window.location.href="s_no.jsp?key="+key+"&Famous=1"; 
	}
	else
		alert("请输入关键字");
}
