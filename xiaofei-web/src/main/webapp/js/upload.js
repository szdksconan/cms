...
*/ 
(function ($) {  
    var defaultsettingss = {  
        url: "",                                 //上传地址  
        buttonFeatures: true,                    //true:点击按钮时仅选择文件； false:选择完文件后立即上传  
        fileSuffixss: ["jpg", "png"],             //允许上传的文件后缀名列表  
        errorTexts: "不能上传后缀为 {0} 的文件！", //错误提示文本，其中{0}将会被上传文件的后缀名替换  
        onCheckUploads: function (text) { //上传时检查文件后缀名不包含在fileSuffixss属性中时触发的回调函数，(text为错误提示文本)  
            alert(text);  
        },  
        onCompletes: function (msg) { //上传完成后的回调函数[不管成功或失败，它都将被触发](msg为服务端的返回字符串)  
        },  
        onAllCompletes: function () {  
        },//全部文件上传完成触发的事件  
  
        onChosens: function (file, obj, fileSize, errorTexts) { //选择文件后的回调函数，(file为选中文件的本地路径;obj为当前的上传控件实例;fileSize为当前文件的大小,errorTexts为获取文件大小时的错误提示文本)  
            //alert(file);  
            return true;//在此回调中返回false将取消当前选择的文件  
        },  
        maximumFilesUploads: 4,//最大文件选择数(当此属性大于1时，buttonFeatures属性只能为true)
        submitFilesNum: 3,//最大提交上传数（当触发submitUpload方法时，文件上传的个数）  
        onSubmitHandles: function (uploadFilesNumsber) { //提交上传时的回调函数，uploadFileNumber为当前上传的文件数量  
            //在此回调中返回false上传提交将被阻止  
            return true;  
        },  
        onSameFilesHandles: function (file) { //当重复选择相同的文件时触发  
            //在此回调中返回false当前选择的文件将从上传队列中取消  
            return true;  
        },  
        isGetFileSizes: false,//是否获取文件大小，默认为false  
          
        isSaveErrorFiles:true,//是否保存上传失败的文件，默认true  
  
        perviewElementIds: "",//用于预览的元素id（请传入一个div元素的id）  
  
        perviewImgStyles: null//用于设置图片预览时的样式（可不设置，在不设置的情况下多文件上传时只能显示一张图片），如{ width: '100px', height: '100px', border: '1px solid #ebebeb' }  
    };  
  
    $.fn.uploadFiles= function (settingss) {  
  
        settingss = $.extend({}, defaultsettingss, settingss || {});  
  
        if (settingss.perviewElementIds) {  
            //设置图片预览元素的必须样式  
            if (!settingss.perviewImgStyles) {  
                var perviewImg = document.getElementById(settingss.perviewElementIds);  
        ...
        return this.each(function () {  
            ...
            var upload = new UploadAssist(settingss);  
            ...
            self.data("uploadFileDatas", upload);  
...
//上传辅助类  
function UploadAssist(settingss) {  
    //保存设置  
...
UploadAssist.prototype = {  
    ...
    createInputFile: function () {  
        ...
        input.onchange = function () {  
            ...
            //保存已经选择的文件路径  
            that.choseFilePaths.push({ "name": this.name, "value": this.value });  
  
            var fileSuf = this.value.substring(this.value.lastIndexOf(".") + 1);  
  
            //检查是否为允许上传的文件  
            if (!that.checkFileIsUpload(fileSuf, that.settingss.fileSuffixss)) {  
                that.removeFile(this.name);  
                that.settingss.onCheckUploads(that.settingss.errorTexts.replace("{0}", fileSuf));  
                return;  
            }  
            ...
            var errorTxt = null;  
            //是否获取上传文件大小  
            if (that.settingss.isGetFileSizes) {  
                fileSize = perviewImage.getFileSize(this, dcmt);  
                if (fileSize == "error") {  
                    fileSize = 0;  
                    errorTxt = that.errorTexts;  
                }  
            }  
  
            //选中后的回调  
            var chosenStatus = that.settingss.onChosens(this.value, this, fileSize, errorTxt);  
            ...
            if (that.checkFileIsExist(this.value)) {  
                var status = that.settingss.onSameFilesHandles(this.value);  
            ...
            //是否开启了图片预览  
            if (that.settingss.perviewElementIds) {  
                if (!that.settingss.perviewImgStyles) {  
                    perviewImage.beginPerview(this, that.settingss.perviewElementIds, dcmt, fileSuf);  
                } else {  
                    var ul = perviewImage.getPerviewRegion(that.settingss.perviewElementIds);  
                    var main = perviewImage.createPreviewElement(this.name, this.value, that.settingss.perviewImgStyles);  
            ...
            if (!that.settingss.buttonFeatures) {  
                that.submitUpload();  
            }  
        }; 
        var formName = "form" + randomNum;  
        var form = $('<form method="post" target="iframe' + randomNum + '" enctype="multipart/form-data" action="' + that.settingss.url+"?num="+that.num++ + '" name="' + formName + '"></form>');  
        form.append(input);  
        ...
        $(dcmt.body).append($("<div></div>").append(form)  
            .append($("<iframe name='iframe" + randomNum + "'></iframe>").on("load", function () {  
                ...
                if (dcmt1.body.innerHTML) {  
                    ...
                    if (responseText == "error" && that.settingss.isSaveErrorFiles) {  
                        //保存上传失败的文件  
                        that.uploadErrors.push(this.name.replace("iframe", "input"));  
                    }  
                    ...
                    if (obj) {  
                        //是否开启了预览  
                        if (that.settingss.perviewElementIds) {  
                            var closeImg = $("#" + that.settingss.perviewElementIds).find("img[name='" + obj.name + "']");  
                            closeImg.next().hide();  
                    ...
                    if (that.settingss.onCompletes) {  
                        that.settingss.onCompletes(dcmt1.body.innerHTML);  
                    }  
  
                    if (that.uploadNums == that.uploadFilesNums) {  
                        that.submitStatus = true;  
                        that.clearUploadQueue();  
                        that.uploadFilesNums = 0;  
                        that.uploadNums = 0;  
                        that.settingss.onAllCompletes();  
                    }  
    ...
    //forEach迭代函数  
    forEach: function (/*数组*/arrs, /*代理函数*/fn) {  
        var len = arrs.length;  
        for (var i = 0; i < len; i++) {  
            var tmp = arrs[i];  
    ...
    submitUpload: function () {  
        var status = this.settingss.onSubmitHandles(this.choseFilePaths.length);  
        ...
        var sbmtNum = this.settingss.submitFilesNum;  
        var len = this.choseFilePaths.length;  
        var dcmt = this.getIframeContentDocument();  
        var that = this;  
        ...
        this.uploadFilesNums = advisableSubmitNum;  
        ...
        for (var i = 0; i < advisableSubmitNum; i++) {  
            (function (n) {  
                ...
                window.setTimeout(function () {  
                    ...
                    if (that.settingss.perviewElementIds) {  
                        //用于设置上传loading图片显示   
                        var imgclose = $("#" + that.settingss.perviewElementIds).find("img[name='" + obj.name + "']");  
                        imgclose.next().show();  
    ...
    insideOperation: function () {  
        ...
        if (!len) return;  
        var obj = this.choseFilePaths[this.uploadFilesNums];  
  
        if (obj && obj.name) {  
            this.uploadFilesNums++;  
            (function(o) {  
                window.setTimeout(function () {  
                    ...
                    if (that.settingss.perviewElementIds) {  
                        //用于设置上传loading图片显示   
                        var imgclose = $("#" + that.settingss.perviewElementIds).find("img[name='" + o.name + "']");  
                        imgclose.next().show();  
                        imgclose.css("visibility", "hidden");  
                    }  
                }, 300);  
    ...
    checkFileIsExist: function (/*当前上传的文件*/file) {  
        ...
        var status = false;  
        this.forEach(this.choseFilePaths, function (i, n) {  
            if (n.value == file) {  
                status = true;  
                return false;  
            }  
    ...
    removeFile: function (name) {  
        ...
        this.forEach(this.choseFilePaths, function (i, n) {  
            if (n.name == name) {  
                ...
                that.choseFilePaths.splice(i, 1);  
                return false;  
            }  
        });  
    },  
    ...
    getObjectByName: function (name) {  
        var obj, that = this;  
        this.forEach(this.choseFilePaths, function (i) {  
            if (this.name === name) {  
                obj = that.choseFilePaths[i];  
                return false;  
            }  
        });  
        return obj;  
    ...
    clearUploadQueue: function () {  
        var len = this.uploadErrors.length;  
        var that = this;  
        if (!len) {  
            this.choseFilePaths.length = 0;  
            this.getIframeContentDocument().body.innerHTML = "";  
        } else {  
            var errorFiles = this.uploadErrors.join();  
            var newArr = this.choseFilePaths.slice(0);  
    ...
    chooseFile: function () {  
        var uploadfile;  
        if (!this.choseFilePaths.length && this.settingss.perviewElementIds) {  
            $("#" + this.settingss.perviewElementIds).find("ul").empty();  
        }  
        if (this.choseFilePaths.length == this.settingss.maximumFilesUploads) {  
            if (this.settingss.maximumFilesUploads <= 1) {  
                this.choseFilePaths.length = 0;  
                var files = this.getIframeContentDocument().getElementsByTagName("input");  
                if (!files.length) {  
