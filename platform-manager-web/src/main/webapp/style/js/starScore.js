jQuery.fn.rater	= function(options) {
	...
	var settings = {
		...
		title_format	: null,
		info_format	: null,
		image	: '../style/img/stars.jpg',
		imageAll :'../style/img/stars-all.gif',
		defaultTips :true,
		clickTips :true,
		width	: 24,
		height	: 24
	...
	//显示结果区域
	var result= jQuery('<div id="startValue" class="rater-star-result"></div>');
	container.after(result); 
	//显示点击提示
	var clickTips= jQuery('<div id="startTip" class="rater-click-tips"><span>点击星星设定渠道星级</span></div>');
	...
	// 当前选中的
	var item	= jQuery('<li id="selectValue" class="rater-star-item-current"></li>');
...
function initStart(stValue){
	$("#selectValue").css("width",(stValue*24)+"px");
	$("#startValue").html(stValue+"级");
	$("#startTip").remove();
}

function getStartValue(){
	var star_value = $("#startValue").html();
	if(star_value==null||star_value==""){
		return 0;
	}else{
