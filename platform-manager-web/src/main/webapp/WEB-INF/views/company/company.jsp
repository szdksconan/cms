	...
	dataGrid = $('#dataGrid').datagrid({
		...
		frozenColumns : [ [ {
			...
			formatter : function(value, row, index) {
				var str = '';
				/*if ($.view) {
					str += $.formatString('<a href="javascript:void(0)" onclick="viewFun(\'{0}\');" ><span style="color:red">查看</span></a>', row.companyId);
				}*/
				if ($.audit && row.status==0){
					str += '&nbsp;|&nbsp;';
					str += $.formatString('<a href="javascript:void(0)" onclick="auditFun(\'{0}\');" ><span style="color:red">审核</span></a>', row.companyId);
				}
				...
				if ($.rePwd && row.status!=1){
					str += '&nbsp;|&nbsp;';
					str += $.formatString('<a href="javascript:void(0)" onclick="rePwd(\'{0}\',0);" ><span style="color:red">重置管理密码</span></a>', row.companyId);
				}
				return str;
			}
		} ] ],
