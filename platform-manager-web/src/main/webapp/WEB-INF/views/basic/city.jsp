...
<head>
<c:if test="${fn:contains(sessionInfo.accessList, '/city/delete')}">
	<script type="text/javascript">
		$.canDel = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.accessList, '/city/edit')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
...
<title>公告管理</title>
	...
	function addFun() {
		parent.$.modalDialog({
			...
			width : 600,
			height : 300,
			href : '${ctx}/city/addPage',
			buttons : [ {
				text : '保存',
				handler : function() {
					parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
					var f = parent.$.modalDialog.handler.find('#cityAddForm')
					f.submit();
				}
			},{
				text : '关闭',
	...
	function editFun(id) {
		...
		buttons : [ {
			...
			handler : function() {
				parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
				var f = parent.$.modalDialog.handler.find('#cityEditForm');
				f.submit();
			}
		},{
			text : '关闭',
...
<body class="easyui-layout" data-options="fit:true,border:false">
	...
	<div data-options="region:'center',border:false" >
   	...
   	<div id="toolbar" style="display: none;">
			<c:if test="${fn:contains(sessionInfo.accessList, '/city/add')}">
