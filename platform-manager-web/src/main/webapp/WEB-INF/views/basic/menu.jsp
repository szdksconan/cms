...
<c:if test="${fn:contains(sessionInfo.accessList, '/menu/edit')}">
	<script type="text/javascript">
		$.canEdit = true;
	</script>
</c:if>
<c:if test="${fn:contains(sessionInfo.accessList, '/menu/delete')}">
	<script type="text/javascript">
		$.canDelete = true;
	</script>
</c:if>
	...
	function editFun(id) {
		...
		if (node) {
			parent.$.modalDialog({
				...
				width : 500,
				height : 350,
				href : '${ctx}/menu/editPage?id=' + node.id,
				buttons : [ {
					text : '保存',
					handler : function() {
	...
	function deleteFun(id) {
		...
		if (node) {
			parent.$.messager.confirm('提示', '您是否要删除当前资源？删除当前资源会连同子资源一起删除!', function(b) {
				if (b) {
					progressLoad();
					$.post('${pageContext.request.contextPath}/menu/delete', {
						id : node.id
					}, function(result) {
						if (result.success) {
...
<body>
	...
	<div id="toolbar" style="display: none;">
		<c:if test="${fn:contains(sessionInfo.accessList, '/menu/add')}">
