...
(function($){
    ...
    BootstrapTable.prototype.init = function () {
        ...
        //if (that.options.editable) {//liuxin 2016-4-12 如果table配置不可编辑则影响翻页功能，所以注释。
    ...
    BootstrapTable.prototype.initBody = function () {
        ...
        //if (!that.options.editable) return;//liuxin 2016-4-12 如果table配置不可编辑则影响翻页功能，所以注释。
        this.initEdit();
    ...
    BootstrapTable.prototype.initEdit = function(){
        ...
        this.$body.find('td').unbind('click').on('click',function(){
            var $tr = $(this);
            ...
            recover(that);
            that.prevEditRow = $tr;
            if (!that.columns['column'+(that.prevEditRow.prevAll().length)].editable)
                return;
            $tr.addClass('editable-select');//给当前编辑行添加样式，目前样式为空只做标识使用
            //that.$body.find('> tr').not(this).removeClass('editable-select');
            that.$body.find('td').not(this).removeClass('editable-select');
        ...
        $(document).bind('mousedown',function(event){
            ...
            if(!($target.parents().andSelf().is(that.$body)) && !($target.parents().andSelf().is($('.laydate_box')))){
                setTimeout(function () {
                    recover(that);
    ...
    function initTrClick(that,_this){
        ...
        if(!tdOpt.edit.type || tdOpt.edit.type == 'text'){
            ...
            $td.html('<div style="margin:0;padding:0;overflow:hidden;border:solid 0px red;height:'+(height)+'px;width:'+(width)+'px;" class="input-group date form_datetime" data-link-field="dtp_editable_input">'
                +'<input class="form-control laydate-icon" id="'+Math.floor(Math.random()*1000)+'_Rom_'+new Date().getTime()+'" type="text" value="'+value+'">'
                //+'<span class="input-group-addon"><span class="glyphicon glyphicon-th"></span></span>'
                +'</div>'
                +'<input type="hidden" id="dtp_editable_input" value="'+value+'"/>'
            );
            that.$body.find('.form_datetime input').each(function () {
                laydate($.extend(tdOpt.edit.layObj,{
                    elem: '#'+$(this).attr('id')
                }));
            });
    ...
    function recover(that){
        ...
        if(that.prevEditRow != null){
            ...
            that.prevEditRow.html(function(i,html){
                ...
                if(textVal != undefined){
                    if($(this).data('oldVal') != (hiddenVal?hiddenVal:$.trim(textVal)) && $(this).data('field')) {
                        ...
                        that.data[that.prevEditRow.parent().data('index')][$(this).data('field')] = hiddenVal?hiddenVal:$.trim(textVal);
                        var index = $(this).parent().children().index($(this));//当前点击的td下标
                        var _arry = [];
                        _arry[0]=textVal;
                        _arry[1]=that.data[that.prevEditRow.parent().data('index')];
                        var _str = 0;
                        if(that.columns['column'+index].otherFormatter != undefined){
							_str=_FunObjectValue(that.columns['column'+index].otherFormatter,that.columns['column'+index],_arry);
                        }
                        
                        $.each(that.columns, function(index) {
				    		if(that.columns[index].total==true){
				    			var fieldIndex = that.columns[index].fieldIndex;
				    			var field = that.columns[index].field;
				    			$this.parent().children().each(function(a){
				    				if(a==fieldIndex){
                                        if (_str!=''){
                                            that.data[that.prevEditRow.parent().data('index')][field] = parseFloat(_str).toFixed(2);
                                            $(this).html(parseFloat(_str).toFixed(2));
                                        }
				    				}
				    			})
				    		}
                        ...
                        isModi = true;
                    }else if ($(this).data('oldVal') == (hiddenVal?hiddenVal:$.trim(textVal))){
                        that.data[that.prevEditRow.parent().data('index')][$(this).data('field')] = $.trim(textVal);
                        isModi = true;
                    }
                    //if(that.columns['column'+i].edit.required == true){
                    if(that.columns['column'+(that.prevEditRow.prevAll().length)].edit.required == true){
            ...
            that.prevEditRow = null;
            //that.$body.find('> tr').removeClass('editable-select');formatter
            that.$body.find('td').removeClass('editable-select');
        }
    }
	var _FunObjectValue = function(_func,_this,_array){
		if (typeof _func === 'function'){
			return _func.apply(_this, _array);
		}else {
			return '';
		}
	};
