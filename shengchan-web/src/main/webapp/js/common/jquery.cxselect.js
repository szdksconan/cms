...
(function(factory) {
  ...
  cxSelect.init = function(dom, settings) {
    ...
    theSelect.settings = $.extend({}, $.cxSelect.defaults, settings, {
      ...
      jsonData:theSelect.dom.box.data('jsonData'),
  ...
  cxSelect.buildOption = function(index, data) {
    ...
    var _jsonData = typeof _select.data('jsonData') === 'undefined' ? self.settings.jsonData : _select.data('jsonData');
    ...
    if (typeof _jsonName === 'string' && _jsonName.length) {
      ...
      for (var i = 0, l = data.length; i < l; i++) {
         var _data = String(data[i][_jsonData])||"";
        _html += '<option value="' + String(data[i][_jsonValue]) + '" data="'+_data+'">' + String(data[i][_jsonName]) + '</option>';
      };
