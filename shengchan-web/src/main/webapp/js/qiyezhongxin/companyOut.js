				...
				success : function(data, textStatus) {
					if (data.success) {
						...
						ShowMap(data.list.base.compZuobiao, data.list.base.companyName,
								data.list.base.address, data.list.base.tel, data.list.base.fax,
								'20');
						//
						...
						var zizhi = $('#zizhi');
						var qy = $('#qy');
						$.each(data.list.contact,function(i, v) {
							if(i<4){
								var lianxi = "<div class='lianxi-l1'>" +
								"<input type='hidden' name='ywid' value="+v.id+">"
						...
						$.each(	data.list.attach,
										function(i, att) {
											...
											if (att.relType == "1") {
												...
												'<img src='+att.attaUrl+'>'+
												'<span class="thumbtext">'+att.describes+'</span></div>';
											...
											if (att.relType == "2") {
												var jiagong = "<div class='swiper-slide' data='fangda'><img src="
														+ att.attaUrl
														+ ' data='+att.id+' onclick="openImg(this)"> </div>';
												jia.append(jiagong);
											}
											if (att.relType == "4") {
						...
						$.each(
								data.list.mainProduct,
								function(i, main) {
									var html = '<div class="swiper-slide">'+
									'<img src='+main.fileurl+'>'+
									'<span class="thumbtext">'+main.setIntroduce+'</span></div>';
									$('#qyproduct').append(html);
								});
