...
var Iput = {
    ...
    clear: function () {
        Iput.confg.positions = "absolute"; Iput.confg.hand = "0"; Iput.confg.ok = null; Iput.confg.top = 0; Iput.confg.left = 0; Iput.confg.bodyHeight = 0; Iput.confg.bodyWidth = 0; Iput.confg.width = 0; Iput.confg.pop = null;
    },
    ...
    pop: function () {
        ...
        $c.style.width = "0px";
        $c.style.height = "0px";
        $c.style.position = Iput.confg.positions;
        $b.setAttribute('id', Iput.confg.idBox);
        $b.setAttribute('align', 'left');
        $b.style.position = Iput.confg.positions;
        $b.style.background = 'transparent';
        $b.style.zIndex = '20000000';
        if ($a) {
        ...
        if (Iput.get(Iput.confg.idIframe)) {
            if (Iput.confg.hand == "1") {
               ...
               /* if (a.firstChild.offsetHeight + top + c > bodyHith) {
                ...
                if(bodywidth-left-a.firstChild.offsetWidth<200){
                	left = left - a.firstChild.offsetWidth+d;
                }
                b.style.top = top - st + "px";
    ...
    colse: function () {
        if (_appendObject){
            if (_appendObject.divId){
                if (_appendObject.appendHtml){
                    var _city = _this.value;
                    ...
                    if (_hproper){
                        ...
                        $('#'+_appendObject.divId).append($.formatString(_appendObject.appendHtml,_city,_dataId,_city));
                    }
                    if ($('.del').length>0){
                        $('.del').click(function () {
                            $(this).parent().remove();
        ...
        if (!document.all) {
            window.document.removeEventListener("click",Iput.hide, false);
        }else {
            window.document.detachEvent("onclick", Iput.hide);
        }
    },
